...
public class SecurityServiceImpl implements SecurityService {
    ...
    private static final Logger logger = Logger.getLogger("SecurityServiceImpl");
    ...
    */
    ...
    /* (non-Javadoc)
      ...
      */
	@Override
	public void signMessage(SOAPMessage soapMessage) throws Exception {
        logger.info("start signing");

        SOAPPart soapPart = soapMessage.getSOAPPart();

        soapPart.getEnvelope().addNamespaceDeclaration("wsse",
                "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd");
        soapPart.getEnvelope().addNamespaceDeclaration("wsu",
                "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd");
        soapPart.getEnvelope().addNamespaceDeclaration("ds",
                "http://www.w3.org/2000/09/xmldsig#");

        soapMessage.getSOAPBody().addAttribute(new QName(
                "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd", "wsu:Id"), "body");

		WSSecHeader  header = new WSSecHeader();
        header.setActor("http://smev.gosuslugi.ru/actors/smev");
        header.setMustUnderstand(false);

        Element sec = header.insertSecurityHeader(soapPart);
        ...
        logger.info("start access to certificate");
        X509Certificate cert = getCertificate();
        logger.info("finish access to certificate");
        ...
        logger.info("start access to private key");
        PrivateKey privateKey = getPrivateKey();
        logger.info("finish access to private key");
	...
	private synchronized KeyStore loadKeyStore() throws Exception {
        if(keyStore == null) {
			keyStore = KeyStore.getInstance(keyStorePath, "JCP");
